...
# 🟢 수신처는 기존 네이트 메일 유지, SMTP 서버는 구글로 지정
...
def parse_naver_news(html_text: str, keyword: str, limit: int = 30) -> list[Article]:
    articles: list[Article] = []
...
def search_articles(start: dt.date, end: dt.date, max_articles: int = 30) -> list[Article]:
    ...
    for keyword in DEFAULT_KEYWORDS:
        ...
        try:
            articles.extend(parse_naver_news(fetch_url(url), keyword, limit=max_articles))
        except Exception as exc:
            articles.append(
                Article(
...
def render_report(articles: list[Article], report_date: dt.date, widened: bool) -> str:
    # ⏰ 현재 한국 시간 기준 오전/오후 타이틀 태그 동적 생성
    kst_now = dt.datetime.utcnow() + dt.timedelta(hours=9)
    time_tag = "[오전 리포트]" if kst_now.hour < 12 else "[오후 리포트]"

    lines = [
        f"{time_tag} [네이버 개발 뉴스 리포트] {format_naver_date(report_date)}",
        "",
        f"기준: {format_naver_date(report_date)} 보도 기사 (발송시간: {kst_now.strftime('%H:%M')} KST)"
...
def load_smtp_config(env: dict[str, str] | None = None) -> SmtpConfig:
    source = env or os.environ
    # 🟢 로그인 ID 및 발신 주소를 구글 계정 주소로 완벽하게 통일하여 550 Blocked 오류 원천 차단
    ...
    if not password:
        raise ConfigError("SMTP_PASSWORD is required. Put the Google app password in GitHub Secrets.")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Send Naver development news report by email.")
    ...
    # ⏰ 현재 한국 시간 기준 제목 태그 동적 접두사 추가
    kst_now = dt.datetime.utcnow() + dt.timedelta(hours=9)
    time_tag = "[오전 리포트]" if kst_now.hour < 12 else "[오후 리포트]"
    subject = f"{time_tag} [개발 뉴스] {format_naver_date(report_date)} 네이버 개발 관련 뉴스"
